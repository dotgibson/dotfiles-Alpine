...
`dotfiles-Alpine` is the **OS-native layer for Alpine Linux** in an **eleven-repo dotfiles system** built on a three-layer
...
- `zsh/zshenv.zsh` — `~/.zshenv` shim exporting `ZDOTDIR`; silences Alpine's
